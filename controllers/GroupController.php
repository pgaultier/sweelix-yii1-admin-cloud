	...
	*/
	public function buildBreadcrumb($currentGroupId) {
		if(isset($this->_breadCrumb[$currentGroupId]) === false) {
			...
			$this->_breadCrumb[$currentGroupId] = array(
				array(
		...
		return $this->_breadCrumb[$currentGroupId];
	}
		...
		try {
			...
			if(isset($_POST[Html::modelName($this->currentGroup)]) === true) {
				...
				if($this->currentGroup->validate() === true) {
					if(($this->currentGroup->groupType === 'single') && ($this->currentGroup->groupType != $previousGroupType)) {
