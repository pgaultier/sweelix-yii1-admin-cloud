...
class TagController extends Controller
{
    ...
    {
        if (isset($this->breadCrumb[$tagId]) === false) {
            $tag = Tag::model()->findByPk($tagId);
            $this->breadCrumb[$tagId] = array(
                array(
        ...
        try {
            ...
            if (Yii::app()->request->isAjaxRequest === true) {
                $this->renderPartial('_listNode', array(
                    'tag' => $this->currentTag,
                    'nodesDataProvider' => $nodeCriteriaBuilder->getActiveDataProvider(array(
                        'pagination' => array(
